...
file_path = 'Results_txt/param_fitting_results_th00.txt'
sys.stdout = open(file_path, "w")

print("####################################################")
print("Parameter Fitting Theta=0 deg case (Parallel)")
print("####################################################")
print("####################################################")
print()
print()
...
for ind in range(2):
    ...
    for j in range(2):
        ...
        while i < tot_param:
            ...
            plt.title(r"a=1 m, H0=1 A/m, $\theta$=0 deg")
            plt.grid('on')
            plt.legend()
            # plt.show()
            if ind==0:
                if j==0:
                    plt.savefig("Plots/Fitting/Parallel_Case/Cubic_Poly/Parameters/a_1_H0_1_th_0_" + param_str[i] + ".png", bbox_inches='tight')
                elif j==1:
                    plt.savefig("Plots/Fitting/Parallel_Case/Cubic_Poly/Parameters/a_1_H0_1_th_0_" + param_str[i] + "_susceff.png", bbox_inches='tight')
            elif ind==1:
                if j==0:
                    plt.savefig("Plots/Fitting/Parallel_Case/Cubic_Poly/Parameters/vary_a_1_H0_1_th_0_" + param_str[i] + ".png", bbox_inches='tight')
                elif j==1:
                    plt.savefig("Plots/Fitting/Parallel_Case/Cubic_Poly/Parameters/vary_a_1_H0_1_th_0_" + param_str[i] + "_susceff.png", bbox_inches='tight')
            plt.close()
